...
### 3. Read-only / view-only share link
...
### 5. Presence-aware node indicators
...
### 6. Cross-node references (non-hierarchical links)
...
**Effort:** ~3–4 days. Needs Supabase Storage (new integration, not currently used anywhere in this app), an upload UI, and thumbnail rendering on the card. The largest "medium" item mainly because it's genuinely new infrastructure, not just new UI.

### 9. Org-wide template/board gallery
...
### 10. Change history / activity log
